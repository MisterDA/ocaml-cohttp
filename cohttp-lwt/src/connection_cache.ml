...
module Make_no_cache (Connection : S.Connection) : sig
  ...
  let create ?(ctx = Lazy.force Net.default_ctx) () ?headers ?body
    ...
    >>= fun connection ->
    let absolute_form =
      match absolute_form with
      | None -> (
          match Net.proxy ~ctx with Some (`GET, _) -> true | _ -> false)
      | Some b -> b
    in
    let res =
      ...
      Lwt.catch
        (fun () ->
          let absolute_form =
            match absolute_form with
            | None -> (
                match Net.proxy ~ctx:self.ctx with
                | Some (`GET, _) -> true
                | _ -> false)
            | Some b -> b
          in
          Connection.call conn ?headers ?body ~absolute_form meth uri)
        (function
          | Retry -> (
              match body with
